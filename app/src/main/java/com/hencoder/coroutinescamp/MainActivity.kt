...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 直接使用 Thread 切线程
        Thread {
            println("test01,Thread io1 ${Thread.currentThread().name}")
            runOnUiThread {
                println("test01,Thread ui1 ${Thread.currentThread().name}")
            }
        }.start()

        // 直接使用 Thread 切线程
        thread {
            println("test02,Thread io2 ${Thread.currentThread().name}")
            runOnUiThread {
                println("test02,Thread ui2 ${Thread.currentThread().name}")
            }
        }

        // 启动一个协程
        GlobalScope.launch {
            println("Coroutines Camp 0 ${Thread.currentThread().name}")
        }
